...
export class ValidateUpdateUnavailabilityMiddleware implements NestMiddleware {
  ...
  /**
   * **summary**: calculate the [**range**](https://www.mathsisfun.com/data/range.html) of the requested Unavailability edit
  ...
  private validateDto = async (value: ValidateUpdateUnavailabilityDto) => {
    ...
    if (value.newStart >= value.newEnd) {
      throw new Error(
        'start time cannot be the same as, or after, the end time',
      );
    }
